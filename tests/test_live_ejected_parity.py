...
_GLOBAL_RECURSIVE_APP = App("global-recursive-app")


@_GLOBAL_RECURSIVE_APP.tool(name="count_down")
def _global_recurse(value: int) -> int:
    global _global_recurse
    return 0 if value == 0 else 1 + _global_recurse(value - 1)
...
def _recursive_app() -> App:
    ...
    def recurse(value: int) -> int:
        def step(current: int) -> int:
            return 0 if current == 0 else 1 + recurse(current - 1)
...
def _shadowing_app() -> App:
    app = App("shadowing-app")

    @app.tool()
    def echo(echo: str) -> str:
        return echo.upper()

    @app.tool(name="renamed_echo")
    def original(value: str) -> str:
        original = value
        return original.upper()

    @app.tool(name="nested_echo")
    def nested_original(value: str) -> str:
        def inner(nested_original: str) -> str:
            return nested_original.upper()
...
def test_live_and_ejected_interfaces_preserve_renamed_self_references() -> None:
    ...
    for api in (live_api, generated_api):
        response = TestClient(api).post("/value", json=3)
        assert response.status_code == 200
    ...
    for mcp in (live_mcp, generated_mcp):
        result = asyncio.run(mcp.call_tool("value", {"value": 3}))
        assert _mcp_text(result) == "3"


def test_live_and_ejected_interfaces_preserve_shadowed_function_names() -> None:
    from fastapi.testclient import TestClient
    ...
    runner = CliRunner()
    live_cli = build_typer_app(app.name, app._tools)
    generated_cli = _ejected(app, "cli", "generated_shadowing_cli").app
    for cli in (live_cli, generated_cli):
        for tool_name, _ in cases:
            result = runner.invoke(cli, [tool_name, "hello"])
            assert result.exit_code == 0, result.exception
            assert result.stdout == "HELLO\n"

    live_api: Any = build_fastapi_app(app.name, app._tools)
    generated_api: Any = _ejected(app, "api", "generated_shadowing_api").app
    for api in (live_api, generated_api):
        client = TestClient(api)
        for tool_name, _ in cases:
            response = client.post(f"/{tool_name}", json="hello")
            assert response.status_code == 200
            assert response.json() == "HELLO"

    live_mcp: Any = build_fastmcp_app(app.name, app._tools)
    generated_mcp: Any = _ejected(app, "mcp", "generated_shadowing_mcp").mcp
    for mcp in (live_mcp, generated_mcp):
        for tool_name, parameter_name in cases:
            result = asyncio.run(mcp.call_tool(tool_name, {parameter_name: "hello"}))
            assert _mcp_text(result) == "HELLO"


def test_live_and_ejected_interfaces_preserve_explicit_global_self_references() -> None:
    from fastapi.testclient import TestClient

    app = _GLOBAL_RECURSIVE_APP
    runner = CliRunner()

    live_cli = build_typer_app(app.name, app._tools)
    generated_cli = _ejected(app, "cli", "generated_global_recursive_cli").app
    for cli in (live_cli, generated_cli):
        result = runner.invoke(cli, ["3"])
        assert result.exit_code == 0, result.exception
        assert result.stdout == "3\n"
    ...
    live_api: Any = build_fastapi_app(app.name, app._tools)
    generated_api: Any = _ejected(app, "api", "generated_global_recursive_api").app
    for api in (live_api, generated_api):
    ...
    live_mcp: Any = build_fastmcp_app(app.name, app._tools)
    generated_mcp: Any = _ejected(app, "mcp", "generated_global_recursive_mcp").mcp
    for mcp in (live_mcp, generated_mcp):
